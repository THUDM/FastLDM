...
def generate_trt(model, inputs, experiment_name=''):
    ...
    with torch.no_grad():
        ...
        del outputs
        os.system("rm trt/{}.onnx".format(name))
        torch.onnx.export(model, inputs, 'trt/{}.onnx'.format(name), operator_export_type=OperatorExportTypes.ONNX_FALLTHROUGH, input_names=['input_{}'.format(i) for i in range(len(inputs))], output_names=['output_{}'.format(i) for i in range(num_output)])
    os.system("rm trt/{}.trt".format(name))
    os.system("trtexec --onnx=trt/{}.onnx --saveEngine=trt/{}.trt --fp16".format(name, name))
    return 'trt/'+name+'.trt'

def experiment(models, trt_models, inputs, experiment_name='', n_iter=100):
