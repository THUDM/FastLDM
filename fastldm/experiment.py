...
def generate_trt(model, inputs, kw_args={}, experiment_name='', onnx_only=False, use_fp16=True, skip=False, onnx_hook=None):
    os.makedirs('./onnx/', exist_ok=True)
    os.makedirs('./trt/', exist_ok=True)
    name = '{}_ONNX_ONLY_{}_{}'.format(type(model).__name__, ONNX_ONLY, experiment_name)
    onnx_path = 'onnx/{}.onnx'.format(name)
    trt_path = 'trt/{}.trt'.format(name)
    if skip:
        if onnx_only:
            return onnx_path
    ...
    with torch.no_grad():
        ...
        torch.onnx.export(model, tuple(inputs)+(kw_args,), onnx_path, operator_export_type=OperatorExportTypes.ONNX_FALLTHROUGH, input_names=['input_{}'.format(i) for i in range(len(inputs)+len(kw_args))], output_names=['output_{}'.format(i) for i in range(num_output)])
        if onnx_hook is not None:
            onnx_hook(onnx_path)
        if onnx_only:
            return onnx_path
    os.system("rm {}".format(trt_path))
    os.system("trtexec --onnx={} --saveEngine={} --buildOnly {} {}".format(onnx_path, trt_path, '--fp16' if use_fp16 else '', PLUGIN_config))
    return onnx_path, trt_path
...
def experiment_trt(model, inputs, kw_args={}, new_inputs=None, new_kw_args=None, use_fp16=True, skip=False, onnx_hook=None):
    _, trt_path = generate_trt(model, inputs, kw_args=kw_args, use_fp16=use_fp16, skip=skip, onnx_hook=onnx_hook)
